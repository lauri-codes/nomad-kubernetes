...
export function handleBackground(background?: string, dim = false): CSSProperties {
  ...
  const style: CSSProperties = {
    ...
    backgroundImage: isColor
      ...
      : background
        ? dim
          ? `linear-gradient(#0006, #0008), url(${resolveAssetUrl(background)})`
...
export function isImageBackground(background?: string) {
  if (!background)
    return false
  return !['#', 'rgb', 'hsl'].some(v => background.indexOf(v) === 0)
}
